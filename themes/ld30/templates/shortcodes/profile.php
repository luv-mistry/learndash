...
<div class="<?php learndash_the_wrapper_class(); ?>">
	...
	<div id="ld-profile" data-shortcode_instance="<?php echo $learndash_shortcode_data_json; ?>">
		...
		<div class="ld-item-list ld-course-list">
			...
			<?php
					$args = array(
						'post_type'      => 'sfwd-courses', // The post type for LearnDash courses
						'post_status'    => 'publish',      // Limit to published courses
						'posts_per_page' => -1,             // Retrieve all courses (-1 means no limit)
					);
					$check = true;
					...
					if ( $allcourses ) :
						foreach ( $allcourses as $course ) :
							if (!empty(get_user_meta( $user_id, 'learndash_course_expired_'.$course->ID, true ))):
								$flag = false ;
								foreach ( $user_courses as $learndash_course_id ){
									if ($course->ID == $learndash_course_id){
										$flag = true;
									}
								}
								if ($flag == false){
								...
								$check = false ;
								learndash_get_template_part(
									'shortcodes/profile/expired-row.php',
									array(
										'user_id'        => $user_id,
										'course_id'      => $course->ID,
										'quiz_attempts'  => $quiz_attempts,
